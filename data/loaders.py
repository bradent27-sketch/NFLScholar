...
@st.cache_data(ttl=CACHE_TTL_SECONDS)
def _newest_external_season(stem, directory='external_data'):
    """Newest season for which `external_data/{stem}_{year}.csv` exists.

    These are hand-exported public-page scrapes with no feed behind them, so
    "which season am I actually looking at" is a real question the app has to
    be able to answer rather than assume.
    """
    import glob as _glob
    years = []
    for path in _glob.glob(os.path.join(directory, f'{stem}_*.csv')):
        tail = os.path.basename(path)[len(stem) + 1:-4]
        if tail.isdigit():
...
def load_sumersports_tendency_data():
    ...
    YEAR-AWARE since 2026-09-02. The filenames were hardcoded to _2025, so a
    freshly exported _2026 file would never have been picked up and the app
    would have served last season's tendencies indefinitely without saying so.
    Now the newest available season wins and the season actually used is
    reported back, so the UI can label a stale read instead of passing it off
    as current.

    Returns (frames, source_year). `source_year` is None when nothing was
    found at all.
    """
    stems = {
        'def_overview': 'sumersports_defensive_overview',
        'off_overview': 'sumersports_offensive_overview',
        'def_personnel': 'sumersports_defensive_personnel_tendency',
        'off_personnel': 'sumersports_offensive_personnel_tendency',
        'def_formation': 'sumersports_defensive_formation_tendency',
        'off_formation': 'sumersports_offensive_formation_tendency',
    }
    source_year = _newest_external_season(stems['def_overview'])
    out = {}
    for key, stem in stems.items():
        path = (os.path.join('external_data', f'{stem}_{source_year}.csv')
                if source_year else None)
        out[key] = pd.read_csv(path) if path and os.path.exists(path) else pd.DataFrame()
    return out, source_year
...
def load_sharp_positional_coverage():
    ...
    Coverage Stats by Position" page
    (external_data/sharp_positional_coverage_{year}.csv). Powers the
    "opposing defense" half of the Defensive Coverage Correlator radar
    (data/coverage_radar.build_defense_radar_data).

    NO LONGER FEEDS ANY PROJECTION. Player Search's next-game projection used
    to scale itself by an alignment multiplier built from this file; as of
    2026-09-02 that surface reads the weekly model instead, so this export is
    descriptive-panel input only.

    YEAR-AWARE since 2026-09-02 - see load_sumersports_tendency_data. Returns
    (frame, source_year); source_year is None when no export exists.
    """
    source_year = _newest_external_season('sharp_positional_coverage')
    if source_year is None:
        return pd.DataFrame(), None
    path = os.path.join('external_data', f'sharp_positional_coverage_{source_year}.csv')
    return (pd.read_csv(path) if os.path.exists(path) else pd.DataFrame()), source_year
