...
# blowout, decided by 28+ either direction - a winning blowout benches
# starters late, and a losing blowout just as often pulls a veteran out of a
# lost cause or shuffles in backups once the outcome is no longer in doubt
# (2026-09-15: reported on Cleveland's WR1 logging a suppressed ~60% snap
# share in a 28+ point road loss to Jacksonville, versus his normal ~85%).
...
MODEL_FEATURES = (
    ...
    'v2_defense_blowout_discount',  # down-weight (not exclude) a defense's
                             # own team-game evidence in build_team_game_
                             # quality_adjusted_matchup when that game was a
                             # 28+ point blowout, either direction - the
                             ...
                             # DEFAULT_FEATURES 2026-09-16, RB/QB/TE (see
                             # _blowout_weeks_for_matchup's
                             # `pos not in ('RB', 'QB', 'TE')` gate).
                             ...
                             # WR/START-WR (+0.001 / +0.027 MAE) - WR stays
                             # excluded on that confirmed cost. QB/TE never
                             # left noise at either scope, but START-QB/
                             # START-TE both pointed the same favorable
                             # direction as RB (-0.022 / -0.012 MAE) - pushed
                             # live at the user's explicit request on that
                             # basis, not a CI confirmation like RB's. Rather
                             # than try a better blowout definition to save WR
                             # (see 'v2_defense_blowout_discount_progressive'
                             # below - it didn't), the fix shipped is a
                             # position gate.
...
DEFAULT_FEATURES = frozenset({
    ...
    # SHIPPED 2026-09-16, RB/QB/TE (see _blowout_weeks_for_matchup's
    # `pos not in ('RB', 'QB', 'TE')` gate inside build_weekly_projections) -
    # down-weight a defense's own 28+-point-blowout team-game evidence when
    # building that position's matchup matrix. Full 26-week confirm
    # (2024+2025 wk5-17, scripts/backtest_component.py --add, bootstrap CI):
    # START-RB -0.019 MAE, CI[-0.042,-0.000] - a real, CI-confirmed win.
    # Ungated (all positions) the SAME run also showed a real, CI-excludes-0
    # WR/START-WR loss (+0.001 / +0.027 MAE) - WR stays excluded on that
    # confirmed cost. QB/TE never left noise at either scope (full-pool or
    # startable), but showed no confirmed cost either, and START-QB/START-TE
    # both pointed the same favorable direction as RB (-0.022 / -0.012 MAE,
    # just under-powered at n=581/459) - pushed live at the user's explicit
    # request 2026-09-16 on that basis, not a CI confirmation like RB's;
    # revisit if a larger window moves either CI off zero. A richer "was this
    # game decided" definition (v2_defense_blowout_discount_progressive,
    # quarter-checkpoint thresholds instead of final margin alone) was built
    # and backtested to try to fix WR without losing the RB win - it did
    # neither: every scope's CI spanned 0, washing out the RB win along with
    # the WR loss rather than separating them. So the surgical fix is
...
def _team_game_quality_profile(game, stats, as_of_week, recency_floor=0.0,
    ...
    if '_script_weight' in game.columns:
        ...
        # A column (not a separately-indexed argument) so it survives the
        # partition_keys recursion above, which splits `game` via groupby
        # before ever reaching this branch.
        weights = weights * pd.to_numeric(game['_script_weight'], errors='coerce').fillna(1.0)
...
# game from contaminating a PLAYER's own rate history; this is the same idea
# for a DEFENSE's own allowed-rate profile
# (build_team_game_quality_adjusted_matchup / _team_game_quality_profile),
# which had no equivalent. A defense up or down 28+ often faces a script
# (a soft prevent shell protecting the lead, an opponent running clock or
# pulling ITS OWN starters once the outcome is decided) that is not
# representative of a competitive game, and that game's raw allowed totals
...
#
# SHIPPED in DEFAULT_FEATURES 2026-09-16, RB/QB/TE: the full 26-week confirm
...
# every position - WR stays excluded on that confirmed cost. QB/TE never left
# noise at either scope, but START-QB/START-TE both pointed the same
# favorable direction as RB (-0.022 / -0.012 MAE, n=581/459) - pushed live at
# the user's explicit request on that basis, not a CI confirmation like RB's;
# revisit if a larger window moves either CI off zero. A richer "was this
# game decided" definition (v2_defense_blowout_discount_progressive,
# quarter-checkpoint thresholds instead of a single final margin) was tried
# to fix WR without losing RB - it washed out BOTH effects instead of
# separating them, so it stays off. The shipped fix is a position gate
# instead: see _blowout_weeks_for_matchup's
# `pos not in ('RB', 'QB', 'TE')` check inside build_weekly_projections.
...
def annotate_player_history_participation(history, name_col, team_col, schedule_df=None):
    ...
    * a proven starter had sharply reduced work in a game decided by 28+
      points (either direction) - a clear late-game-rest situation on the
      winning side, or a lost-cause snap reduction on the losing side -
      rather than an ordinary lower-workload game.
    ...
    # player sat, so use it solely for a huge margin (either direction) plus
    # an already established player's materially reduced measured
    # participation. Symmetric by design: a winning blowout benches starters
    # late, and a losing blowout (2026-09-15: Cleveland's WR1 at ~60% snaps
    # in a 28+ point road loss, versus his normal ~85%) just as often pulls a
    # veteran once the outcome is no longer in doubt - both describe a game
    # that is not a normal upcoming-role sample, not just the winning side.
    margins = _team_week_margins(schedule_df)
    if not margins.empty:
        ...
        blowout_reduced_role = (
            frame['_established_role']
            & frame['_final_margin'].abs().ge(SEVERE_BLOWOUT_MARGIN)
            & frame['_snap'].gt(0.0)
            & frame['_snap'].le(SEVERE_BLOWOUT_MAX_SNAP_SHARE)
            & frame['_snap'].le(SEVERE_BLOWOUT_RELATIVE_MAX_SHARE * frame['_prior_snap_reference'])
        )
        winning_side = blowout_reduced_role & frame['_final_margin'].gt(0.0)
        losing_side = blowout_reduced_role & frame['_final_margin'].lt(0.0)
        mark(frame.loc[winning_side, '_row'].to_numpy(), 'severe blowout rest')
        mark(frame.loc[losing_side, '_row'].to_numpy(), 'severe blowout rest (losing side)')
...
def build_weekly_projections(year, week, scoring_mode='Full PPR', as_of_week=None, apply_injury=True,
    ...
    def _blowout_weeks_for_matchup(schedule_frame, matchup_year):
        ...
        RB/QB/TE, not WR (2026-09-16). The full 26-week confirm backtest
        found a real, CI-confirmed START-RB win and a real, CI-confirmed
        WR/START-WR loss at the shipped 0.5 discount - WR is excluded on that
        confirmed cost. QB/TE never left noise on either scope (full-pool or
        startable), but showed no confirmed cost either, and START-QB/START-TE
        both pointed the same favorable direction as RB (-0.022 / -0.012 MAE,
        just under-powered at this sample) - pushed live on that basis at the
        user's explicit request 2026-09-16, same as RB but without RB's CI
        confirmation. Revisit if a larger-window backtest moves either scope's
    ...
    source_contract['partial_game_history_filter'] = (
        ...
        'paired partial replacements, and severe blowout rest games (either side)'
    )
