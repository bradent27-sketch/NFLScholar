"""v2_cold_start_room_budget backtest (2026-09-07).

Snap share is not conserved to 100% of team snaps - a team runs one, two or
three tight ends on a play, so one room's shares legitimately sum to ~1.3 for
TE / ~2.2 for WR - but it IS bounded, and by a real team signal. Nothing in
the cold-start pipeline bounds the room sum: restoration, the Ourlads floor,
and v2_receiver_cold_start_vacancy (which SUMS every departed rotational
player's prior share, then redistributes 70% of that sum weighted to the
biggest incumbent) can push a room well past 2.0. LAC 2026: three departed
backup TEs (Fisk .35 + Dissly .32 + Conklin .30, never on the field together)
drove a returning TE-2 whose own role barely changed to the 0.92 vacancy cap,
room summing over 2.0.

v2_cold_start_room_budget (apply_cold_start_room_snap_budget) clamps the SUM
of a team's WR (or TE) room shares to what that team actually ran at the
position last year (prior_room_snap_budget) x (1 + COLD_START_ROOM_BUDGET_
TOLERANCE), water-filling the excess out of the players the Ourlads chart
lists as deep reserves first and only scaling the protected top tier if that
is not enough.

Both base and variant carry v2_historical_ourlads so the frozen pre-Week-1
archive and prior-season role reference exist for a historical year. Cold
start (weeks 1-2) - the pass is a no-op once real games are in.

    python scripts/sweep_cold_start_room_budget.py --years 2022,2023,2024,2025 --weeks 1-2
"""
import argparse
import math
import os
...
VAR_FEATS = frozenset(BASE_FEATS | {'v2_cold_start_room_budget'})
        ...
        for week in weeks:
            ...
            for player in touched:
                ...
                ledger.append(dict(
                    ...
                    base_snap=round(float(b.loc[player, 'Expected Snap Share']), 3)
                    if 'Expected Snap Share' in b.columns else None,
                    var_snap=round(float(v.loc[player, 'Expected Snap Share']), 3)
                    if 'Expected Snap Share' in v.columns else None,
...
def _report(scope_pairs, stat_pairs, ledger, years, weeks):
    bar = '=' * 96
    print(f"\n{bar}\nv2_cold_start_room_budget   years={years} weeks={weeks}\n{bar}")
    print("dMAE = variant - base.  NEGATIVE = clamping the room to its prior-year budget is MORE accurate.\n")
    ...
    if not ledger:
        print("none - no team's room exceeded its prior-year budget this window.")
    else:
        for r in sorted(ledger, key=lambda r: (r['year'], r['week'], r['team'], -abs(r['shift']))):
            print(f"  {r['year']} w{r['week']:<3} {r['team']:<4} {r['pos']:<3} {r['player']:<24} "
                  f"snap {r['base_snap']}->{r['var_snap']}  proj {r['base_proj']:>6.2f}->{r['var_proj']:>6.2f} "
                  f"({r['shift']:>+5.2f})  actual={r['actual']}  err {r['base_error']}->{r['var_error']}")
